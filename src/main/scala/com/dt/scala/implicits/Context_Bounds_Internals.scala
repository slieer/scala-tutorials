 ...
 * Tel:	18610086859
 */
class Pair_Implicits[T: Ordering](val first1: T, val second1: T){ //Ordering[T]
  def bigger(implicit ordered: Ordering[T]) =
    if (ordered.compare(first, second) > 0) first else second
}
class Pair_Implicitly[T: Ordering](val first2: T, val second2: T){
	def bigger =if (implicitly[Ordering[T]].compare(first, second) > 0) 
			first else second
}
class Pair_Implicitly_Odereded[T: Ordering](val first2: T, val second3: T){
	def bigger ={
	  import Ordered._
